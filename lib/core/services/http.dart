...
Future<dynamic> httpGet(String path, Map<String, dynamic> data,
  ...
  try {
    ...
    data['longitude'] = longitudeGlobal;
    data['time_zone'] = "";
    String queryString = Uri(queryParameters: data.map((key, value) => MapEntry(key, value.toString()))).query;
    var fullUrl = "$url?$queryString";
    var response = await http.get(Uri.parse(fullUrl), headers: headers).timeout(const Duration(seconds: 15)); // Timeout after 15 seconds
    if (response.statusCode == 200) {
      responsegetData = json.decode(const Utf8Codec().decode(response.bodyBytes));
